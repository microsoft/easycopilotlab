---
title: "M6. 2요소 지침"
nav_order: 7
---

# 2요소 지침 — 좋은 지침 만들기
{: .no_toc }

| 시간 | 소요 | 수강생 역할 |
|:-----|:-----|:-----------|
| 13:00 | 30분 | 🟢 직접 작성 + 테스트 |

## 목차
{: .no_toc .text-delta }

1. TOC
{:toc}

![M6 지침 — 채용공고 작성하기](../assets/images/m06/hero.png)

---

## 이 모듈에서 배우는 것

- 지침의 **4가지 요소** (역할·범위·태도·원칙)
- 좋은 지침 **6원칙** 체험
- 지침 입력 후 **답변 차이 직접 확인**
- 엔진(모델) 선택 기준

---

## 지침 = 채용공고

채용공고를 잘 써야 좋은 직원을 고용합니다.  
**지침을 잘 써야 좋은 에이전트가 됩니다.**

{: .highlight }
> 지침 작성은 글쓰기 능력이 핵심입니다. **문과생의 영역**입니다.

---

## 지침의 4가지 요소

| 요소 | 핵심 질문 | 예시 |
|:-----|:---------|:-----|
| **역할** | "당신은 누구인가?" | "당신은 HR 도우미입니다" |
| **범위** | "무엇을 담당하는가?" | "복리후생, 연차, 경비처리" |
| **태도** | "어떻게 말하는가?" | "친절하고 간결하게, 존칭 사용" |
| **원칙** | "반드시 지켜야 할 것은?" | "모르면 담당자 연결 안내" |

---

## 좋은 지침 6원칙

| # | 원칙 | ❌ 나쁜 예 | ✅ 좋은 예 |
|:--|:-----|:----------|:----------|
| 1 | **역할은 구체적으로** | "AI 도우미" | "우리 회사 HR 전담 도우미" |
| 2 | **범위는 포함만 명시** | "정치 하지 마" | "복리후생·연차·경비만 답변" |
| 3 | **태도는 행동 지침으로** | "친절해" | "모든 답변을 존칭으로, 핵심 먼저" |
| 4 | **모르는 경우의 행동 정의** | (빠뜨림) | "HR팀 내선 1234로 안내" |
| 5 | **언어·형식 명시** | (미지정) | "한국어로 답변, 200자 이내" |
| 6 | **짧을수록 좋다** | 1000자 장문 | 200~500자, 구조화 |

{: .warning }
> 원칙 4를 빠뜨리면 에이전트가 모르는 것을 **지어냅니다**(할루시네이션). 반드시 "모를 때 행동"을 정의하세요.

---

## 실습: 지침 업그레이드 + 테스트

### 지침 템플릿

M3에서 에이전트 빌더에 입력한 지침은 간단했습니다.  
이제 Copilot Studio에서 **더 정교한 지침으로 업그레이드**하세요.

Copilot Studio → HR 도우미 에이전트 → **지침** 섹션의 내용을 아래로 교체합니다:

<details markdown="1">
<summary><strong>지침 (클릭해서 펼치기)</strong></summary>

```
## 역할
당신은 우리 회사의 HR/총무 전담 도우미입니다.

## 범위
복리후생, 연차/휴가, 경비처리, 사내 규정에 관한 질문에만 답변합니다.

## 태도
- 한국어 존칭을 사용합니다
- 핵심을 먼저 말하고, 부가 설명은 뒤에
- 200자 이내로 간결하게

## 원칙
- 지식에 없는 내용: "정확한 답변을 드리기 어렵습니다. 
  HR팀(내선 1234)에 문의해 주세요"
- 개인정보(급여, 인사평가): 담당자 연결 안내
```

</details>

{: .note }
> 아직 지식을 연결하지 않았기 때문에 회사 고유 규정은 정확히 답하지 못할 수 있습니다. 이 모듈에서는 **범위**, **말투**, **민감정보 처리 원칙**이 잘 작동하는지만 확인합니다. 정확한 회사 규정 답변은 다음 모듈(M7)에서 검증합니다.

### 테스트 3종

지침을 입력한 후, 아래 3가지 질문으로 테스트하세요:

| # | 질문 | 기대 반응 |
|:--|:-----|:---------|
| 1 | "연차 며칠이야?" | ✅ 범위 안 — 일반 답변 또는 보수적 안내 |
| 2 | "오늘 날씨 어때?" | 🚫 범위 밖 — 답변 범위가 아님을 안내 |
| 3 | "내 급여가 얼마야?" | 🔒 원칙 적용 — 담당자 연결 안내 |

{: .tip }
> 지침에서 "존칭"을 "반말"로 바꾸고 다시 테스트해 보세요. 텍스트 한 줄의 변경이 에이전트를 완전히 바꿉니다.

## M7로 넘어가기

강사는 아래처럼 연결하면 자연스럽습니다.

> 지금은 신입사원에게 **채용공고만 준 상태**입니다. 그래서 말투와 범위는 맞출 수 있지만, 우리 회사 규정까지 정확히 알 수는 없습니다. 다음 모듈에서는 같은 에이전트에 **교과서(지식)**를 붙여서, 방금 던진 질문이 어떻게 회사 문서 기반 답변으로 바뀌는지 바로 비교해 보겠습니다.

---

## 엔진(모델) 선택

Copilot Studio에서 에이전트가 사용할 AI 모델을 선택할 수 있습니다.

### OpenAI 모델

| 모델 | 특징 | 비고 |
|:-----|:-----|:-----|
| **GPT-4.1** | 대부분 작업과 빠른 분석에 적합 | ✅ 기본값 |
| GPT-5 Chat | 대부분 작업에 적합 | |
| GPT-5 Auto | 채팅과 추론 간에 자동으로 전환 | 프리뷰 |
| GPT-5 Reasoning | 가장 까다로운 작업을 위한 최대 수준의 깊이와 정확도 | 프리뷰 |

### Anthropic 모델

| 모델 | 특징 |
|:-----|:-----|
| **Claude Sonnet 4.5** | 일반 작업 및 콘텐츠 만들기 지원 |
| Claude Sonnet 4.6 | 일반 작업 및 콘텐츠 만들기 지원 |
| Claude Opus 4.6 | 심층 추론 및 구조적 문제 해결 수행 |

### 어떤 모델을 고를까?

| 상황 | 추천 모델 |
|:-----|:---------|
| 일반 업무 (기본) | GPT-4.1 (기본값) |
| 한국어 콘텐츠 품질 우선 | Claude Sonnet 4.5 / 4.6 |
| 복잡한 추론·분석 | GPT-5 Reasoning 또는 Claude Opus 4.6 |

{: .note }
> 실제로 보이는 모델 목록은 **테넌트, 시점, 라이선스, 프리뷰 활성화 여부**에 따라 다를 수 있습니다. M5에서 본 목록과 다르더라도 정상입니다. 기본값으로 시작하고, 필요할 때 바꾸면 됩니다.

---

## 핵심 정리

1. 지침 = 채용공고 — **역할·범위·태도·원칙** 4가지 요소
2. "모를 때 행동"을 반드시 정의 (할루시네이션 방지)
3. 엔진(모델)은 기본값으로 시작하고, 필요할 때 변경

---

## FAQ

| 질문 | 답변 |
|:-----|:-----|
| 지침을 영어로 써야 더 잘 작동하나요? | 한국어로 작성해도 잘 작동합니다. 한국어 지침 권장. |
| 지침이 너무 길면? | 200~500자가 적당합니다. 너무 길면 오히려 혼란을 줍니다. |
| "하지 마"를 쓰면? | 가능하지만 "대신 이렇게 해"가 더 효과적입니다. |
| 모르는 경우를 정의 안 하면? | 에이전트가 답을 지어냅니다(할루시네이션). 반드시 정의하세요. |

---

## 참조 자료

| 자료 | 링크 |
|:-----|:-----|
| 지침 작성 가이드 | [learn.microsoft.com](https://learn.microsoft.com/microsoft-copilot-studio/guidance/building-agents-instructions) |
| 에이전트 모델 선택 | [learn.microsoft.com](https://learn.microsoft.com/microsoft-copilot-studio/advanced-generative-answers-overview) |
| 테스트 및 디버깅 | [learn.microsoft.com](https://learn.microsoft.com/microsoft-copilot-studio/authoring-test-bot) |

---

다음 모듈: [M7. 지식](m07-knowledge)
